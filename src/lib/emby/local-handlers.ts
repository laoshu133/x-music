...
import { enqueueEmbyTrackSync, enqueuePendingEmbyTrackSyncs, requestUserTrackSync } from './sync'
...
import { createLocalAccessToken, findAccountByAccessToken, localEmbyUserId } from './tokens'
...
async function handleAuthenticateByName(request: Request): Promise<Response> {
  ...
  markAccountLogin(account.userId, readRequestIp(request))
...
async function authorizeLocalRequest(request: Request): Promise<AccountRecord | undefined> {
  ...
  markAccountActive(account.userId)
...
function accountForUpstreamFallback(request: Request): AccountRecord | undefined {
  ...
  const token = readClientAccessToken(request)
  if (!token) return undefined
  const matched = findAccountByAccessToken(token)
  if (matched) markAccountActive(matched.userId)
  return matched
}
...
function remoteMappingForVirtualItem(decoded: VirtualId, account?: AccountRecord): RemoteMappingRecord | undefined {
  if (decoded.kind === 'qq-playlist') {
    return getRemoteMapping({ userId: account?.userId, localType: 'playlist', localKey: `qq:${decoded.id}`, remote: 'emby' })
  }
  if (decoded.kind === 'qq-song') {
    return getRemoteMapping({ userId: account?.userId, localType: 'track', localKey: `tx:${decoded.songmid}`, remote: 'emby' })
  }
  return undefined
}
...
function forgetRemoteMappingForVirtualItem(decoded: VirtualId, mapping?: RemoteMappingRecord): void {
  ...
  const remoteId = mapping?.remoteId
  if (decoded.kind === 'qq-playlist') {
    deleteRemoteMapping({ userId, localType: 'playlist', localKey: `qq:${decoded.id}`, remote: 'emby', remoteId })
  } else if (decoded.kind === 'qq-song') {
    deleteRemoteMapping({ userId, localType: 'track', localKey: `tx:${decoded.songmid}`, remote: 'emby', remoteId })
  }
}
  ...
  if (!loaded) {
    if (!favorite) {
      ...
      if (tracked) {
        ...
        setLocalFavorite(song, false, account?.userId)
...
function getMappedSongmidForEmbyItemId(itemId: string, account?: AccountRecord): string | undefined {
  const mapping = getRemoteMappingByRemote({ userId: account?.userId, remote: 'emby', remoteId: itemId })
...
async function handlePlaybackReportRequest(request: Request, embyPath: string): Promise<Response | undefined> {
  ...
  if (stored && /\/Stopped$/i.test(embyPath)) {
    ...
    insertPlayEvent(track.id, quality, account?.userId)
...
function enqueueArchiveAndSyncForSong(
  ...
  if (!account || !shouldArchiveForAccount(account)) return
  const track = ensureTrack(musicInfo)
  requestUserTrackSync(account.userId, track.id, reason)
  enqueueTrackArchive({
  ...
  enqueueEmbyTrackSync(account.userId, {
    source: musicInfo.source,
    songmid: musicInfo.songmid,
    playlistId,
    musicInfo,
...
async function handlePlaylistItemsRequest(request: Request, embyPath: string): Promise<Response | undefined> {
  ...
  if (decoded.kind === 'qq-playlist') {
    const playlist = loadVirtualPlaylist(decoded.id)
    const mapped = getRemoteMapping({ userId: account?.userId, localType: 'playlist', localKey: `qq:${decoded.id}`, remote: 'emby' })?.remoteId
      ?? (playlist ? await searchEmbyPlaylistByName(playlist.name, { account }).catch(() => undefined) : undefined)
    if (mapped) {
      upsertRemoteMapping({ userId: account?.userId, localType: 'playlist', localKey: `qq:${decoded.id}`, remote: 'emby', remoteId: mapped, raw: playlist })
...
async function handlePlaylistItemsMutationRequest(request: Request, embyPath: string): Promise<Response | undefined> {
  ...
  const playlist = loadVirtualPlaylist(decoded.id)
  const mappedPlaylistId = getRemoteMapping({ userId: account?.userId, localType: 'playlist', localKey: `qq:${decoded.id}`, remote: 'emby' })?.remoteId
    ?? (playlist ? await searchEmbyPlaylistByName(playlist.name, { account }).catch(() => undefined) : undefined)
  if (!mappedPlaylistId) return undefined
  upsertRemoteMapping({ userId: account?.userId, localType: 'playlist', localKey: `qq:${decoded.id}`, remote: 'emby', remoteId: mappedPlaylistId, raw: playlist })
...
async function handleAudioRequest(request: Request, embyPath: string): Promise<Response | undefined> {
  ...
  if (shouldProxyUnknownExternalEmbyAudio(itemId, account)) {
    const proxied: Response | undefined = await proxyToUpstreamEmby(request, embyPath).catch((error: unknown) => {
      logVirtualAudioEvent('external_emby_audio_proxy_failed', request, {
        itemId,
        error: errorMessage(error),
      }, 'error')
      return undefined
    ...
    logVirtualAudioEvent('external_emby_audio_proxy_unusable', request, {
      itemId,
      upstreamStatus,
    })
  }
...
function syncedWebdavPlaybackCandidates(musicInfo: MusicInfo, preferredQuality: MusicQuality, account?: AccountRecord): WebdavPlaybackCandidate[] {
  ...
  for (const row of rows) {
    try {
      ...
      if ((payload.userId ?? '') !== (account?.userId ?? '')) continue
...
async function proxyMappedEmbyAudioAfterLxFailure(input: {
  ...
  if (!hasUpstreamEmbyConfigured(input.account)) return undefined
  const mappedItemId = await resolveEmbyTrackMapping(input.musicInfo, input.account)
  if (!mappedItemId) return undefined

  const action = input.embyPath.split('/')[3] ?? 'universal'
  const proxied: Response | undefined = await proxyToUpstreamEmby(input.request, `/Audio/${encodeURIComponent(mappedItemId)}/${action}`).catch((error: unknown) => {
    logVirtualAudioEvent('virtual_audio_mapped_fallback_proxy_failed', input.request, {
      itemId: input.itemId,
      songmid: input.songmid,
      mappedItemId,
      action,
  ...
  const upstreamStatus = proxied?.status
  if (isUsableUpstreamResponse(proxied)) return proxied
  logVirtualAudioEvent('virtual_audio_mapped_fallback_proxy_unusable', input.request, {
    itemId: input.itemId,
    songmid: input.songmid,
    mappedItemId,
    action,
...
async function resolvePlayableUpstreamResponse(
  ...
  const cachedSkippedQualities: MusicQuality[] = []
  let networkAttempts = 0
  let encryptedQQRequiresKey = false

  const tryQuality = async (quality: MusicQuality): Promise<{
    url: string
    quality: MusicQuality
    response: Response
    completion: Promise<void>
  ...
  for (const quality of qualities) {
    ...
    if (cachedUnplayable) {
      cachedSkippedQualities.push(quality)
  ...
  if (networkAttempts === 0 && cachedSkippedQualities.length > 0) {
    for (const quality of cachedSkippedQualities) {
      logVirtualAudioEvent('virtual_audio_quality_retrying_stale_unplayable_cache', request, {
        songmid: musicInfo.songmid,
        preferredQuality,
        quality,
      })
...
function shouldSyncResolvedQualityToEmby(
  ...
  attempts: Array<{ quality: MusicQuality; error: string; source?: string; musicId?: string }>,
): boolean {
...
function localSongmidForExternalItemId(itemId: string, account?: AccountRecord): string | undefined {
  ...
  return directLocalSongmidForExternalItemId(itemId)
}
...
function shouldProxyUnknownExternalEmbyAudio(itemId: string, account?: AccountRecord): boolean {
  if (!itemId || decodeClientVirtualId(itemId)) return false
  if (looksLikeQQSongMid(itemId)) return false
  if (mappingForExternalTrackItemId(itemId, account)) return false
  if (directLocalSongmidForExternalItemId(itemId)) return false
  if (staleEmbyTrackAliasSongmid(itemId)) return false
  return hasUpstreamEmbyConfigured(account)
}

function directLocalSongmidForExternalItemId(itemId: string): string | undefined {
...
function deleteStaleTrackMapping(mapping: Pick<RemoteMappingRecord, 'userId' | 'localType' | 'localKey' | 'remote' | 'remoteId' | 'rawJson'>): void {
  rememberStaleEmbyTrackAlias(mapping)
  deleteRemoteMapping({ userId: mapping.userId, localType: mapping.localType, localKey: mapping.localKey, remote: mapping.remote, remoteId: mapping.remoteId })
}
...
function validEmbyTrackMapping(musicInfo: MusicInfo, account?: AccountRecord): string | undefined {
  const mapping = getRemoteMapping({ userId: account?.userId, localType: 'track', localKey: `${musicInfo.source}:${musicInfo.songmid}`, remote: 'emby' })
...
function unplayableReason(error: unknown): string | undefined {
  ...
  if (error instanceof MusicUrlResolveError) {
    const attempt = error.attempts.find(item => isMusicUrlUnavailableAttempt(item.error))
...
function localFavoriteStateForRequest(request: Request): Map<string, ReturnType<typeof getFavoriteStatusForAccount> & { song?: MusicInfo }> {
  const account = authorizedLocalAccount(request)
  const local = listLocalFavoritesForAccount(account?.userId)
  return new Map(local.map(record => [`${record.source}:${record.songmid}`, {
    ...getFavoriteStatusForAccount(record.source, record.songmid, account?.userId),
    song: record,
  }]))
}
...
function localPlayHistoryToEmbyItems(limit: number, account?: AccountRecord): any[] {
  ...
  for (const event of listPlayHistory(account.userId, limit)) {
