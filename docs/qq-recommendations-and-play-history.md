...
猜你喜欢限制的是每页数量，不是歌单总量。Web API 和 Emby/Ampcast 的单页 `Limit` 上限均为 30；Emby 初始使用 `TotalRecordCount=999` 表示结果仍可继续分页，接近该位置时总量提示会随当前页继续增长。该数值只是未知总量的客户端兼容提示，不是业务总数上限。

Emby 推荐结果使用按 `user_id` 隔离的 60 秒内存池。连续翻页复用已有结果，每次请求最多新增 30 首；直接跳到较大 `StartIndex` 时只加载目标页，不补抓中间页面。相同页在缓存有效期内直接复用，避免 Ampcast 重复刷新产生额外 QQ 请求。
